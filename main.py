...
from util import AttnLabelConverter, make_json
...
def run(target,save_json = False):
    ...
    '''
    data : pred한 단어 저장하는 리스트
    score : confidence score 누적시키는 변수
    img_dir : 이미지 전처리 후 저장 장소
    '''
    data = []
    score = 0
    img_dir = './img'
    ...
    image_data = RawDataset(root='./temp')  # use RawDataset
    image_loader = torch.utils.data.DataLoader(
        image_data, batch_size=128,
        shuffle=False,
        num_workers=int(0),
    ...
    with torch.no_grad():
        for image_tensors, image_path_list in image_loader:
            ...
            log.write(f'{os.path.basename(target[0]):25s}\n{" ".join(data)}\n{score/len(data):0.4f}\n')

            log.close()

            # Save Json
            if save_json == True:
                make_json(img_dir,data,loc[0])
            else:
    ...
    return " ".join(data), score/len(data)


target = ['./test/10-20190207162430645.jpg']
data, conf_score = run(target,True)
print('결과')
print(data)
print()
print(f'score : {conf_score}')
target2 = ['./test/10-20190207170104526.jpg']
data, conf_score = run(target2)
print('결과')
print(data)
print()
print(f'score : {conf_score}')
